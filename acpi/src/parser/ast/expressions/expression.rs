use super::{
    ...
    LNot, LNotEqual, LOr, Load, LoadTable, Match, MethodInvocation, Mid, Mod, Multiply, NAnd, NOr,
    Not, ObjectType, Or, ReferenceTypeOp, ShiftLeft, ShiftRight, SizeOf, Store, Subtract, Timer,
    ToBCD, ToBuffer, ToDecimalString, ToHexString, ToInteger, ToString, Wait, Xor,
};
    ...
    pub(in crate::parser::ast) fn parse_opt(
        ...
        match next!(stream, "Expression") {
            ...
            EXT_OP_PREFIX => match_next!(stream, "Extended Expression",
                ...
                LOAD_OP => Load::parse(stream, context).map(|load| Expression::Load(load)),
...
impl<'a> core::fmt::Display for Expression<'a> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Expression::Load(load) => load.fmt(f),
