...
use base::error::IDE_DRIVER_MODULE_NUMBER;
use device::Device;
...
pub struct IDEController<T: ProcessTypes + 'static> {
...
unsafe fn irq_handler<T: ProcessTypes + 'static>(context: usize) {
    let _controller = &mut *(context as *mut IDEController<T>);
}

impl<T: ProcessTypes + 'static> IDEController<T> {
    ...
    fn enumerate_drives(&mut self) -> base::error::Result<usize> {
        // Install IRQ handlers
        interrupts::irqs::install_irq_handler(
            14,
            irq_handler::<T>,
            self as *mut IDEController<T> as usize,
        );
        interrupts::irqs::install_irq_handler(
            15,
            irq_handler::<T>,
        ...
        for i in 0..2 {
            ...
            for j in 0..2 {
                ...
                if drive_type == DRIVE_TYPE_ATA {
                    ATA::<T>::create(channel.clone(), drive, capabilities, size, model)?;
                } else {
                    ATAPI::<T>::create(channel.clone(), drive, capabilities, size, model)?;
                }
            }
        }
    ...
    fn polling(&mut self, channel: u8, advanced_check: bool) -> base::error::Result<usize> {
...
impl<T: ProcessTypes + 'static> device::Device for IDEController<T> {
    ...
    fn read_register(&mut self, address: usize) -> base::error::Result<usize> {
        ...
        let result = device::inb(if reg < 0x08 {
    ...
    fn ioctrl(&mut self, code: usize, argument: usize) -> base::error::Result<usize> {
        match code {
            ...
            IOCTRL_SET_CHANNEL_INTERRUPT => {
                if argument >= 2 {
                    Err(Box::new(ControllerError::InvalidIOCtrl))
                } else {
...
impl core::fmt::Display for ControllerError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ControllerError::BadAddress => write!(f, "Bad address specified"),
            ControllerError::InvalidIOCtrl => write!(f, "Invalid I/O control for IDE controller"),
        }
    }
}
