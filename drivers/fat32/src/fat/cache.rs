...
impl<T: ProcessTypes + 'static> Cache<T> {
    ...
    fn get_buffer_common(&mut self, sector_offset: usize) -> base::error::Result<&mut FATSector> {
        // Search to see if the sector is cached
        let mut set_location = 0;
        let mut lowest_access_value = usize::MAX;

        for i in 0..CACHE_SIZE {
            let sector = match &mut self.sectors[i] {
                Some(sector) => sector,
                None => {
                    set_location = i;
        ...
        self.drive
            .lock(|drive| drive.read(sector_offset, &mut buffer))?;
        ...
        Ok(&mut self.sectors[set_location].as_ref().unwrap())
    }
}
