...
use base::{bootloader::MemoryDescriptor, critical::CriticalLock};
use core::{
    ops::{Index, IndexMut},
    ptr::null_mut,
};
use interrupts::{exceptions::ExceptionInfo, Registers};

pub type MemoryExceptionHandler = unsafe fn(address: usize);
...
static mut VIRTUAL_MEMORY_INITIALIZED: bool = false;

// These two do not require critical locks because they are set once during boot
static mut NULL_ACCESS_EXCEPTION_HANDLER: MemoryExceptionHandler =
    default_null_access_exception_handler;
static mut INVALID_ACCESS_EXCEPTION_HANDLER: MemoryExceptionHandler =
    default_invalid_access_exception_handler;

static KERNEL_ADDRESS_SPACE: CriticalLock<AddressSpace> = CriticalLock::new(AddressSpace(0));
...
pub unsafe fn initialize(
    ...
    assert!(!VIRTUAL_MEMORY_INITIALIZED);
    VIRTUAL_MEMORY_INITIALIZED = true;

    NULL_ACCESS_EXCEPTION_HANDLER = null_access_exception_handler;
    INVALID_ACCESS_EXCEPTION_HANDLER = invalid_access_exception_handler;
    ...
    interrupts::exceptions::install_exception_handler(0xE, page_fault_handler);
}
...
unsafe fn default_null_access_exception_handler(_: usize) {
    panic!("No null access exception handler setup!");
}

unsafe fn default_invalid_access_exception_handler(_: usize) {
    panic!("No null invalid exception handler setup!");
}

unsafe fn page_fault_handler(_: &Registers, info: &ExceptionInfo) {
    let cr2 = get_cr2();

    if (info.error_code & 1) == 0 {
        if cr2 < PAGE_SIZE {
            NULL_ACCESS_EXCEPTION_HANDLER(cr2);
        } else {
            let mut current_address_space = get_current_address_space();
            current_address_space.allocate(cr2, physical::allocate());
        }
    } else {
        INVALID_ACCESS_EXCEPTION_HANDLER(cr2);
    }
}
...
impl AddressSpace {
    pub fn new() -> Self {
        ...
        let kernel_pml4 = (KERNEL_ADDRESS_SPACE.lock().0 + KERNEL_VMA) as *mut PML4;
        let mut i = 256;
        while i < 512 {
            unsafe {
    ...
    pub fn allocate(&mut self, virtual_address: VirtualAddress, physical_address: PhysicalAddress) {
        ...
        let kernel_address_space_lock = if user {
            super::MEMORY_USAGE.lock().userspace_pages += 1;
            None
        } else {
            Some(KERNEL_ADDRESS_SPACE.lock())
        };
        ...
        drop(kernel_address_space_lock)
    }
